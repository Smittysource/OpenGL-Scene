...
This code was modified to change the mouse scroll functionality to speed up and slow down camera movement.
Modifier:           David Smith
Course:             CS-330
...
public:
    ...
    {
        // Adjust speed of camera change by mouse movement
        MouseSensitivity += yoffset * STEP;
        if (MouseSensitivity < 0.01f) {
            MouseSensitivity = 0.01f;
        }
        if (MouseSensitivity > 3.0f) {
            MouseSensitivity = 3.0f;
        }
        // Adjust speed of camera change by key press
        MovementSpeed += yoffset * STEP * 100;
